...
void hhev_control_region(string inputfilename, int TriggerFlagSel, bool check_comb, float in1, float in2, string filename)
{
...
string out_branch_name = "CR_NOCUT_"+filename; 
...
// Sort by csv, loop on list, pick first three that meet the csv cut
	...
	if(jcount<2) continue;
		for(int l=0; l<numj; l++) {
			if (rdcount>=1) {break;} 
			if(EVJets[l].btag < bTagCSV_looseCut) {
				JETvec[2]=fillTLorentzVector(EVJets[l].pt, EVJets[l].eta, EVJets[l].phi, EVJets[l].e);
				btag_backup[2]=EVJets[l].btag;
				EVJets[l].flag=true;
				hhev.ThirdJetTracks = EVJets[l].nTr;			//already token flag!
				rdcount++;}
		}
	if(rdcount<1) continue;
...
for(int jj=EVJets.size(); jj>=0; jj--)
{
	...
	hhev.APt_max = 0; hhev.APt_min = 0; hhev.APt_mean = 0; hhev.AEta_max = 0; hhev.AEta_min = 0; hhev.AEta_mean = 0;
	hhev.ACSV_max = 0; hhev.ACSV_min = 0; hhev.ACSV_mean = 0;hhev.Acent = 0;
	}
...
ofstream asciifile;
string asciifilename = "stats_CR_"+filename+".txt";
